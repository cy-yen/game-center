<template>
  <van-popup v-model:show="show" position="left" :style="{ width: '60%', height: '100%' }">
    <div class="bg-[#1a3647] h-full p-[15px] text-[15px] text-white leading-tight">
      <section class="flex items-center">
        <van-icon name="arrow-left" class="text-[20px]" />
        <img
          class="w-[36px] ml-[15px]"
          src="data:image/png;base64,"
          alt=""
        />
        <span class="ml-[15px]">未登入</span>
      </section>
      <section class="flex justify-between my-[26px] items-center">
        <div>
          <p class="text-[25px]">$0.000</p>
          <p class="text-[#edcbdd] text-[14px]">餘額</p>
        </div>
        <div class="w-[41px] h-[41px] flex justify-center items-center bg-[#00b7ff] rounded-[100%]">
          <img
            class="w-[20px]"
            src="data:image/png;base64,"
            alt=""
          />
        </div>
      </section>
      <section>
        <ul>
          <li class="border-b-[0.5px] border-b-[#4837f9] py-[15px] px-[20px] flex justify-between items-center">
            <div class="flex leading-[20px]">
              <img
                class="w-[20px]"
                src="data:image/png;base64,"
                alt=""
              />
              <span class="ml-[10px]">控制台</span>
            </div>
            <van-icon name="arrow" class="text-[14px]" />
          </li>
          <li class="border-b-[0.5px] border-b-[#4837f9] py-[15px] px-[20px] flex justify-between items-center">
            <div class="flex leading-[20px]">
              <img class="w-[20px]" src="./../assets/images/btn2.97c07c75.png" alt="" />
              <span class="ml-[10px]">遊戲</span>
            </div>
            <van-icon name="arrow" class="text-[14px]" />
          </li>
          <li class="border-b-[0.5px] border-b-[#4837f9] py-[15px] px-[20px] flex justify-between items-center">
            <div class="flex leading-[20px]">
              <img
                class="w-[20px]"
                src="data:image/png;base64,"
                alt=""
              />
              <span class="ml-[10px]">報表</span>
            </div>
            <van-icon name="arrow" class="text-[14px]" />
          </li>
          <li class="border-b-[0.5px] border-b-[#4837f9] py-[15px] px-[20px] flex justify-between items-center">
            <div class="flex leading-[20px]">
              <img
                class="w-[20px]"
                src="data:image/png;base64,"
                alt=""
              />
              <span class="ml-[10px]">設置</span>
            </div>
            <van-icon name="arrow" class="text-[14px]" />
          </li>
        </ul>
      </section>
      <section class="mt-[26px] flex justify-center items-center">
        <button
          class="border-[0.5px] border-[#ee4581] leading-[41px] rounded-[20px] px-[25px] mr-[10px]"
          @click="$router.push('/login')"
          v-if="!isLogin"
        >
          登錄
        </button>
        <button
          class="border-[0.5px] border-[#ee4581] leading-[41px] rounded-[20px] px-[25px]"
          @click="$router.push('/register')"
          v-if="!isLogin"
        >
          註冊
        </button>
        <button class="border-[0.5px] border-[#ee4581] leading-[41px] rounded-[20px] px-[25px]" v-if="isLogin">
          退出
        </button>
      </section>
    </div>
  </van-popup>
</template>

<script setup>
import { ref } from 'vue'

const show = ref(false)
const isLogin = ref(false)

defineExpose({
  showMenu: () => {
    show.value = true
  }
})
</script>
